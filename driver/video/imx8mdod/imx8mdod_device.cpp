    ...
    )
{
    ...
    *NumberOfChildrenPtr = CHILD_COUNT;     // represents the HDMI connector
    ...
    LOG_TRACE("Successfully started device.");
    return STATUS_SUCCESS;
}
    ...
    )
{
    ...
    LOG_TRACE("Child relations reported successfully.");
    return STATUS_SUCCESS;
}
    ...
    )
{
    ...
    LOG_ERROR("Device descriptor query is not implemented.");
    ...
    )
{
    ...
    if (!SetPointerPositionPtr->Flags.Visible) {
        LOG_TRACE("Received request to set pointer visibility to OFF.");
        return STATUS_SUCCESS;
    } else {
        LOG_ERROR(
    ...
    )
{
    ...
    if (IsSupportedVidPnPtr->hDesiredVidPn == 0) {
        ...
        LOG_TRACE("Returning IsVidPnSupported=TRUE for hDesiredVidPn=0.");
        return STATUS_SUCCESS;
    }
    ...
    while (status == STATUS_SUCCESS) {
               ...
               presentPathPtr->VidPnTargetId)))
        {
                ...
                D3DKMDT_VPPS_UNPINNED)
            {
                ...
                // any stretch modes
                LOG_TRACE("Setting scaling support to Identity.");
               ...
               presentPathPtr->VidPnTargetId)))
        {
            ...
            {
                LOG_TRACE("Setting rotation support to Identity.");
    ...
    )
{
    ...
    if (numPaths == 0) {
        LOG_INFORMATION("There are no paths in this topology.");
        return STATUS_SUCCESS;
    }
    ...
    )
{
    ...
    __try {
        ...
        LOG_ERROR("An exception occurred while accessing the user buffer.");
        return STATUS_UNSUCCESSFUL;
    }
